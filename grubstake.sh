...
cmd_clean() {
    ...
    [ -d "$_root" ] || return 0
    _bad=0
    # The root may not be grubstake's alone to manage, so only names it would itself have created are ever removed.
    for _t in $(known_tools); do
        _e="$_root/$_t"
        [ -e "$_e" ] || [ -L "$_e" ] || continue
        log "removing $_e"
