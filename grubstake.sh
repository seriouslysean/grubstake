...
# Exported for the whole spine, gates included: a gate reaching for any pinned tool must refuse a
# cold cache rather than download mid-commit (AGENTS rule 17), and only this scope covers gates too.
...
# from grubstake.tools directly: grubstake.sh may be a symlink whose pins live beside its target,
# which a literal path missed and left lint silently skipped.
SWIFTLINT=""
if [ -n "$STAGED_SWIFT" ]; then
