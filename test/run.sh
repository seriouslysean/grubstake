...
r=$(new_repo)
head -c "$((_trunc_anchor + ${#_trunc_lit} + 1))" "$GS" >"$r/trunc.sh"
chmod +x "$r/trunc.sh"
out=$(sh "$r/trunc.sh" version 2>&1)
rc=$?
if [ "$rc" -eq 0 ]; then
...
# rc alone does not discriminate: `ensure` exits non-zero either way. The two guard-only properties
# asserted are the arch named on stderr and no "downloading" line.
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 $SHA_A $SHA_A"
_unameshim="$r/uname-shim"
mkdir -p "$_unameshim" || fixture_die "cannot create the uname shim dir"
...
_curlshim="$r/curl-blocked"
mkdir -p "$_curlshim" || fixture_die "cannot create the blocked-curl shim dir"
printf '#!/bin/sh\necho "curl: network blocked in test" >&2\nexit 6\n' >"$_curlshim/curl"
chmod +x "$_curlshim/curl" || fixture_die "cannot make the blocked curl shim executable"
_out=$(cd "$r" && PATH="$_unameshim:$_curlshim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh ensure 2>&1)
_rc=$?
if [ "$_rc" -eq 0 ]; then
...
# install_tool must stop at the platform guard rather than log a per-tool skip for an empty platform.
# The fixture pins only swiftlint, since periphery's genuine linux gap logs the same "not published for" line.
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 $SHA_A $SHA_A"
_unameshim="$r/uname-shim"
mkdir -p "$_unameshim" || fixture_die "cannot create the uname shim dir"
...
printf '#!/bin/sh\necho "curl: network blocked in test" >&2\nexit 6\n' >"$_curlshim/curl"
chmod +x "$_curlshim/curl" || fixture_die "cannot make the blocked curl shim executable"
_out=$(cd "$r" && PATH="$_unameshim:$_curlshim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh check 2>&1)
_rc=$?
if [ "$_rc" -eq 0 ]; then
...
printf '#!/bin/sh\necho "curl: network blocked in test" >&2\nexit 6\n' >"$_curlshim/curl"
chmod +x "$_curlshim/curl" || fixture_die "cannot make the blocked curl shim executable"
_out=$(cd "$r" && PATH="$_unameshim:$_curlshim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh path swiftlint 2>&1)
_rc=$?
...
_out=$(cd "$r" && GRUBSTAKE_CACHE="cache" ./grubstake.sh path swiftlint 2>&1)
_rc=$?
if [ "$_rc" -eq 0 ]; then
...
# GRUBSTAKE_CACHE is left unset so cache_root's own platform check is reached, and the report must
# render in full, stderr-clean, before doctor exits non-zero.
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 $SHA_A $SHA_A"
_unameshim="$r/uname-shim"
uname_arch_shim "$_unameshim"
...
_fakehome="$r/fake-home"
mkdir -p "$_fakehome" || fixture_die "cannot create the fake HOME dir"
_out=$(cd "$r" && env -u GRUBSTAKE_CACHE -u XDG_CACHE_HOME PATH="$_unameshim:$PATH" HOME="$_fakehome" ./grubstake.sh clean 2>&1)
_rc=$?
if [ "$_rc" -eq 0 ]; then
...
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 $SHA_A $SHA_A"
_pout=$(cd "$r" && env -u GRUBSTAKE_CACHE -u XDG_CACHE_HOME HOME="fake-home" ./grubstake.sh path swiftlint 2>&1)
_prc=$?
_cout=$(cd "$r" && env -u GRUBSTAKE_CACHE -u XDG_CACHE_HOME HOME="fake-home" ./grubstake.sh clean 2>&1)
_crc=$?
_eout=$(cd "$r" && env -u GRUBSTAKE_CACHE -u XDG_CACHE_HOME HOME="" ./grubstake.sh path swiftlint 2>&1)
_erc=$?
if [ "$_prc" -eq 0 ]; then
...
_out=$(cd "$r" && GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh check 2>&1)
_rc=$?
if [ "$_rc" -eq 0 ]; then
...
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 darwin=$SHA_A linux=$SHA_B"
fake_install "$r" swiftlint 0.63.2 "$SHA_A"
fake_install "$r" swiftlint 0.63.2 "$SHA_B"
_dshim="$r/uname-darwin"
mkdir -p "$_dshim" || fixture_die "cannot create the darwin uname shim dir"
...
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 darwin=$SHA_A linux=$SHA_A windows=$SHA_A"
fake_install "$r" swiftlint 0.63.2 "$SHA_A"
expect_ok "$r" check

it "a key omitted from a keyed line behaves like a positional '-'"
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 darwin=$SHA_A"
_lshim="$r/uname-linux"
mkdir -p "$_lshim" || fixture_die "cannot create the linux uname shim dir"
cat >"$_lshim/uname" <<'SHIM'
#!/bin/sh
...
SHIM
chmod +x "$_lshim/uname" || fixture_die "cannot make the linux uname shim executable"
_out=$(cd "$r" && PATH="$_lshim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh path swiftlint 2>&1)
_rc=$?
if [ "$_rc" -eq 0 ]; then
...
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 darwin=$SHA_A linux=nothex"
_out=$(gs "$r" check)
_rc=$?
if [ "$_rc" -eq 0 ]; then
...
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 darwin=$SHA_A darwin=$SHA_B"
_out=$(gs "$r" check)
_rc=$?
if [ "$_rc" -eq 0 ]; then
...
_out=$(cd "$r" && GRUBSTAKE_CACHE="$r/.cache" dash ./grubstake.sh check 2>&1)
_rc=$?
if [ "$_rc" -eq 0 ]; then
...
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 $SHA_A\cJUNK $SHA_B"
_out=$(cd "$r" && GRUBSTAKE_CACHE="$r/.cache" dash ./grubstake.sh check 2>&1)
_rc=$?
if [ "$_rc" -eq 0 ]; then
...
git -C "$(dirname "$0")/.." grep -qiE "tamper" -- . ':!test/run.sh'
_tamper_rc=$?
case "$_tamper_rc" in
...
pins "$r" "swiftlint 0.63.2 $SHA_A $SHA_A"
fake_install "$r" swiftlint 0.63.2 "$SHA_A"
rm -f "$r/.cache/swiftlint/$SHA_A/swiftlint"
expect_fail "$r" check
...
_out=$(cd "$r" && PATH="$r/curl-shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh ensure 2>&1)
_rc=$?
if [ "$_rc" -eq 0 ]; then
...
if [ "$_rc" -ne 0 ]; then
    ...
    _crc=$(
        cd "$r" && GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh check >/dev/null 2>&1
        echo $?
    )
...
for _case in absent partial; do
    ...
    _rc=$?
    if [ "$_rc" -ne 0 ]; then
        fail "state '$_case': ensure exited $_rc: $_out"
        _bad=1
        break
    fi
    _crc=$(
    ...
    if [ "$_crc" -ne 0 ]; then
        fail "state '$_case': ensure exited 0 but check still fails afterward"
        _bad=1
        break
    fi
done
...
# Published entries are hardened read-only, so clean must reclaim write permission before rm -rf can succeed.
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 $SHA_A $SHA_A"
fake_install "$r" swiftlint 0.63.2 "$SHA_A"
...
elif [ -e "$r/.cache/swiftlint" ]; then
    fail "the read-only entry was left behind: $_out"
else
    pass
fi

it "an unremovable partial directory fails loudly instead of nesting the staging"
# publish_dir's chmod -R u+w cannot restore search permission it was never asked to add, so the debris
# survives rm -rf, and mv would nest the staging inside it, since macOS has no mv -T.
r=$(new_repo)
_sha=$(fake_release "$r" 0.63.2)
pins "$r" "swiftlint 0.63.2 $_sha $_sha"
...
mkdir -p "$_cache"
_out=$(cd "$r" && PATH="$r/curl-shim:$PATH" GRUBSTAKE_CACHE="$_cache" ./grubstake.sh ensure 2>&1)
_rc=$?
...
staging_failure_fixture
_mvshim="$r/mv-shim"
mkdir -p "$_mvshim" || fixture_die "cannot create $_mvshim"
...
_out=$(cd "$r" && PATH="$_mvshim:$r/curl-shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh ensure 2>&1)
_rc=$?
n=$(find "$r/.cache" -name '*.staging.*' 2>/dev/null | wc -l | tr -d ' ')
if [ "$_rc" -eq 0 ]; then
...
staging_failure_fixture
_cpshim="$r/cp-shim"
mkdir -p "$_cpshim" || fixture_die "cannot create $_cpshim"
...
_out=$(cd "$r" && PATH="$_cpshim:$r/curl-shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh ensure 2>&1)
_rc=$?
n=$(find "$r/.cache" -name '*.staging.*' 2>/dev/null | wc -l | tr -d ' ')
if [ "$_rc" -eq 0 ]; then
...
staging_failure_fixture
_unzipshim="$r/unzip-shim"
mkdir -p "$_unzipshim" || fixture_die "cannot create $_unzipshim"
...
_out=$(cd "$r" && PATH="$_unzipshim:$r/curl-shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh ensure 2>&1)
_rc=$?
n=$(find "$r/.cache" -name '*.staging.*' 2>/dev/null | wc -l | tr -d ' ')
if [ "$_rc" -eq 0 ]; then
...
SHIM
    ...
    _out=$(cd "$r" && PATH="$_shim:$PATH" GRUBSTAKE_CACHE="$_degenerate" ./grubstake.sh clean 2>&1)
    _rc=$?
    if [ "$_rc" -eq 0 ]; then
        fail "clean exited 0 for GRUBSTAKE_CACHE=$_degenerate: $_out"
        _bad=1
        break
    elif [ -f "$_record" ]; then
        fail "the guard let a destructive command run before refusing (GRUBSTAKE_CACHE=$_degenerate): $(cat "$_record")"
        _bad=1
        break
    fi
done
...
it "clean removes only tool entries and leaves a user file in the cache root"
# The cache root may be a shared or otherwise populated directory, not one clean can claim outright.
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 $SHA_A $SHA_A"
fake_install "$r" swiftlint 0.63.2 "$SHA_A"
printf 'cache-root 1\n' >"$r/.cache/.grubstake-cache-root" || fixture_die "cannot write the fixture sentinel"
printf 'do not delete me\n' >"$r/.cache/notes.txt" || fixture_die "cannot write $r/.cache/notes.txt"
_out=$(gs "$r" clean)
_rc=$?
if [ "$_rc" -ne 0 ]; then
    fail "clean refused a cache root holding only a known tool entry and a user file: $_out"
elif [ -e "$r/.cache/swiftlint" ]; then
    fail "the tool entry survived clean: $_out"
elif [ "$(cat "$r/.cache/notes.txt" 2>/dev/null)" != "do not delete me" ]; then
    fail "the user file at the cache root did not survive clean: $_out"
elif [ ! -f "$r/.cache/.grubstake-cache-root" ]; then
    fail "an unrelated file at the cache root did not survive clean: $_out"
elif [ ! -d "$r/.cache" ]; then
    fail "the cache root itself did not survive clean: $_out"
else
    pass
fi

it "clean leaves a directory it does not recognise"
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 $SHA_A $SHA_A"
fake_install "$r" swiftlint 0.63.2 "$SHA_A"
printf 'cache-root 1\n' >"$r/.cache/.grubstake-cache-root" || fixture_die "cannot write the fixture sentinel"
mkdir -p "$r/.cache/other/nested" || fixture_die "cannot create $r/.cache/other/nested"
printf 'do not delete me\n' >"$r/.cache/other/file.txt" || fixture_die "cannot write $r/.cache/other/file.txt"
_out=$(gs "$r" clean)
_rc=$?
if [ "$_rc" -ne 0 ]; then
    fail "clean refused a cache root holding an unrecognised directory: $_out"
elif [ -e "$r/.cache/swiftlint" ]; then
    fail "the tool entry survived clean: $_out"
elif [ ! -f "$r/.cache/other/file.txt" ] || [ "$(cat "$r/.cache/other/file.txt")" != "do not delete me" ]; then
    fail "the unrecognised directory did not survive clean: $_out"
else
    pass
fi

it "clean follows a symlinked cache root and keeps the link"
# Removal happens beneath the link, not on it, so the link and its target both have to survive.
r=$(new_repo)
rm -rf "$r/.cache" || fixture_die "cannot remove the fixture .cache directory"
_real="$r/real-cache"
mkdir -p "$_real/swiftlint/$SHA_A" || fixture_die "cannot create $_real/swiftlint/$SHA_A"
printf '#!/bin/sh\necho 0.63.2\n' >"$_real/swiftlint/$SHA_A/swiftlint" || fixture_die "cannot write the fixture binary"
chmod +x "$_real/swiftlint/$SHA_A/swiftlint" || fixture_die "cannot make the fixture binary executable"
ln -s "$_real" "$r/.cache" || fixture_die "cannot symlink the cache root"
_out=$(gs "$r" clean)
_rc=$?
if [ "$_rc" -ne 0 ]; then
    fail "clean refused a symlinked cache root: $_out"
elif [ ! -L "$r/.cache" ]; then
    fail "the symlink itself did not survive clean: $_out"
elif [ ! -d "$_real" ]; then
    fail "the symlink's target directory did not survive clean: $_out"
elif [ -e "$_real/swiftlint" ]; then
    fail "the tool entry inside the symlinked root survived clean: $_out"
else
    pass
fi

it "clean accepts a cache root spelled with a trailing slash"
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 $SHA_A $SHA_A"
fake_install "$r" swiftlint 0.63.2 "$SHA_A"
_out=$(cd "$r" && GRUBSTAKE_CACHE="$r/.cache/" ./grubstake.sh clean 2>&1)
_rc=$?
if [ "$_rc" -ne 0 ]; then
    fail "clean refused a cache root spelled with a trailing slash: $_out"
elif [ -e "$r/.cache/swiftlint" ]; then
    fail "the tool entry survived clean: $_out"
else
    pass
...
it "clean exits non-zero when a tool entry cannot be removed"
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 $SHA_A $SHA_A"
fake_install "$r" swiftlint 0.63.2 "$SHA_A"
_shim="$r/rm-shim"
mkdir -p "$_shim" || fixture_die "cannot create the rm shim dir"
...
# Checked on stderr alone, not combined output: "removing $_root/\$_t" is a log() line on stdout that
# names the root regardless of whether the rm after it actually succeeds.
(cd "$r" && PATH="$_shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh clean >/dev/null 2>"$r/err")
_rc=$?
_err="$(cat "$r/err" 2>/dev/null)"
if [ "$_rc" -eq 0 ]; then
    fail "clean exited 0 despite a tool entry that could not be removed"
elif ! printf '%s\n' "$_err" | grep -qF "$r/.cache"; then
    fail "clean failed but stderr never named the cache root: $_err"
else
    pass
fi
...
(
    cd "$r" && PATH="$_shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh ensure >"$r/out" 2>"$r/err"
    echo $? >"$r/rc"
) &
_bgpid=$!
_w=0
...
_cleanout=$(cd "$r" && GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh clean 2>&1)
_cleanrc=$?
[ "$_cleanrc" -eq 0 ] || fixture_die "clean itself failed while racing ensure: $_cleanout"
[ -e "$r/.cache/swiftlint" ] && fixture_die "clean did not actually remove the tool entry; the race window is not real"
: >"$_go"
...
# An untraversable ancestor must be named as a permission failure, not a removed cache; the mkdir
# shim flips it only at the lock attempt, after install_tool's earlier -x check.
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 $SHA_A $SHA_A"
fake_install "$r" swiftlint 0.63.2 "$SHA_A"
_shim="$(mktemp -d "$ROOT/lock-count.XXXXXX")" || fixture_die "cannot create the lock-count shim dir"
...
# A concurrent exec of the same hash path must never see it vanish, so ensure refuses and tells the
# human how to recover instead of touching the entry.
r=$(new_repo)
_sha=$(fake_release "$r" 0.63.2)
pins "$r" "swiftlint 0.63.2 $_sha $_sha"
_out=$(cd "$r" && PATH="$r/curl-shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh ensure 2>&1)
...
printf '#!/bin/sh\necho "curl: network blocked in test" >&2\nexit 6\n' >"$_shims/curl"
chmod +x "$_shims/curl"
_out=$(cd "$r" && PATH="$_shims:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh ensure 2>&1)
_rc=$?
...
# The incident guard from CONTRIBUTING.md's "When a fix changes behaviour on upgrade": verify_tool/check stay
# existence-only, so a receiptless legacy entry must pass check without waiting for ensure to re-touch it.
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 $SHA_A $SHA_A"
...
r=$(new_repo)
_d="$r/dest"
_st="$r/dest.staging.333"
mkdir -p "$_st"
printf 'staged-bytes' >"$_st/swiftlint"
chmod +x "$_st/swiftlint"
mkdir -p "$_d"
printf 'original-bytes' >"$_d/swiftlint"
chmod +x "$_d/swiftlint"
...
printf 'corrupted-bytes' >"$_d/swiftlint" # mismatched: bytes no longer match the receipt beside them
{
    ...
    echo 'publish_dir "$1" "$2" "$3"'
} >"$r/t.sh"
_out=$(cd "$r" && sh "$r/t.sh" "$_st" "$_d" swiftlint 2>&1)
_rc=$?
...
if [ "$_rc" -ne 0 ]; then
    ...
    _out2=$(cd "$r" && PATH="$_shims:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh ensure 2>&1)
    _rc2=$?
    if [ "$_rc2" -ne 0 ]; then
...
if [ "$_rc" -eq 0 ]; then
    ...
    # The diagnostic records swiftformat's entry and lock state, to tell contention from an unrelated mkdir failure.
...
# swiftformat's own backfill line is the evidence it was reached, and no ok line may appear on this red run.
...
xcbeautify 1.0.0 $SHA_A $SHA_A"
    _cout=$(cd "$r" && GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh check 2>&1)
    _crc=$?
...
# Repo A execs the shared binary for the whole of repo B's ensure, and the catch is probabilistic,
# so a green run shows only that this run hit no destroy-in-place window.
a=$(new_repo)
b=$(new_repo)
...
_out=$(cd "$b" && PATH="$a/curl-shim:$PATH" GRUBSTAKE_CACHE="$_shared" ./grubstake.sh ensure 2>&1)
_rc=$?
...
a=$(new_repo)
b=$(new_repo)
pins "$a" "swiftlint 0.63.2 $SHA_A $SHA_A"
fake_install "$a" swiftlint 0.63.2 "$SHA_A"
pins "$b" "swiftlint 0.63.2 $SHA_B $SHA_B"
fake_install "$b" swiftlint 0.63.2 "$SHA_B"
...
_out=$(cd "$r" && PATH="$_shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" GRUBSTAKE_OFFLINE=1 ./grubstake.sh path swiftlint 2>&1)
_rc=$?
if [ "$_rc" -eq 0 ]; then
...
_sha=$(fake_release "$r" 0.63.2)
pins "$r" "swiftlint 0.63.2 $_sha $_sha"
_out=$(cd "$r" && PATH="$r/curl-shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh path swiftlint 2>&1)
_rc=$?
if [ "$_rc" -ne 0 ]; then
...
mkdir -p "$_tmpdir"
_out=$(cd "$r" && TMPDIR="$_tmpdir" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh update 0.1.4 2>&1)
_rc=$?
if [ -e "$_sentinel" ]; then
...
# The verb must replace the target and stop without running it. The replacement is a copy of $0, so
# the marker lives in the invoking script, gated on an "ensure" argument only the auto-ensure supplies.
r=$(new_repo)
...
# Each name's own shape decides how it is pulled: a one-liner closes on its opening line, so
# only a name still spanning multiple lines needs the range read through to its own closing brace.
_helpers=""
for _hn in log die; do
    _hline_no="$(grep -n "^$_hn() {" "$GS" | head -1 | cut -d: -f1)"
    [ -n "$_hline_no" ] || fixture_die "extract $_hn: no '$_hn() {' line in $GS"
    _hline="$(sed -n "${_hline_no}p" "$GS")"
    case "$_hline" in
        *"}") _helpers="$_helpers
$_hline" ;;
        *) _helpers="$_helpers
$(sed -n "${_hline_no},/^}\$/p" "$GS")" ;;
    esac
done
...
chmod +x "$r/t.sh"
_marker="$r/EXECUTED"
_out=$(cd "$r" && GST_TEST_MARKER="$_marker" "$r/t.sh" "$r/target.sh" 9.9.9 2>&1)
_rc=$?
...
(cd "$r" && sh "$r/t.sh" "$_st" "$_d" swiftlint) >/dev/null 2>&1
_rc=$?
...
if [ "$_rc" -ne 0 ]; then
    fail "the generated script exited $_rc, so publish_dir did not run"
elif [ -e "$_st" ]; then
    fail "the loser could not remove its own read-only staging"
elif [ "$(cat "$_d/swiftlint" 2>/dev/null)" != "$_before" ]; then
    fail "the winner's own executable was disturbed"
else pass; fi
chmod -R u+w "$r" 2>/dev/null

it "a race loser's staging cleanup that rm cannot finish is reported in grubstake's voice, not raw rm stderr"
# A loser whose staging rm cannot finish must report it in grubstake's voice; lock_pause_shim holds
# the loser at its lock while the winner publishes, and the plant makes the loser's staging half-unremovable.
r=$(new_repo)
_sha=$(fake_release "$r" 0.63.2)
pins "$r" "swiftlint 0.63.2 $_sha $_sha"
_dest="$r/.cache/swiftlint/$_sha"
...
(
    cd "$r" && PATH="$r/curl-shim:$_shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh ensure >"$r/b.out" 2>&1
    echo $? >"$r/b.rc"
) &
_bpid=$!
_w=0
...
_outA=$(cd "$r" && PATH="$r/curl-shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh ensure 2>&1)
_rcA=$?
...
r=$(new_repo)
_d="$r/dest"
_st="$r/dest.staging.222"
mkdir -p "$_st"
printf 'bin' >"$_st/swiftlint"
chmod +x "$_st/swiftlint"
...
if [ "$_rc" -ne 0 ]; then
    ...
    fail "the staging directory was not moved: $_st still exists"
elif [ ! -x "$_d/swiftlint" ]; then
    fail "debris was cleared, but the staging was never published into $_d"
else pass; fi
chmod -R u+w "$r" 2>/dev/null
...
if [ "$_rc" -ne 1 ]; then
    fail "the generated script exited $_rc, expected the locked command's 1"
...
# A signal while write_receipt holds the lock must still release it; mv_pause_shim holds the
# receipt rename so the kill lands with the lock held, and the orphaned shim is reaped.
r=$(new_repo)
pins "$r" "swiftlint 0.63.2 $SHA_A $SHA_A"
fake_install "$r" swiftlint 0.63.2 "$SHA_A"
_shim="$r/mv-shim"
...
# with_lock must restore install_tool's own trap rather than clobber it, or a signal mid-publish
# leaves the staging it names; this test cannot also catch a stranded lock, which the write_receipt
# test covers; mv_pause_shim is gated on the publish destination.
r=$(new_repo)
_sha=$(fake_release "$r" 0.63.2)
pins "$r" "swiftlint 0.63.2 $_sha $_sha"
_dest="$r/.cache/swiftlint/$_sha"
...
_out=$(cd "$r" && PATH="$_shim:$PATH" sh "$r/t.sh" 2>&1)
_rc=$?
_want="0.10.0
0.9.9
0.2.0"
...
_out=$(cd "$r" && PATH="$_shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" GRUBSTAKE_RAW="file://$f/raw" ./grubstake.sh update 2>&1)
_rc=$?
_after="$(cat "$r/grubstake.sh")"
if [ "$_rc" -ne 0 ]; then
...
_out=$(cd "$r" && GIT_CONFIG_GLOBAL=/dev/null GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh install 2>&1)
_rc=$?
...
# Two installs pinned together at the hooksPath write by git_pause_shim must neither corrupt each
# other's hook tmp files nor leak a raw tool error; hooksPath is unset each iteration so every round
# reaches that write; the hook-tmp half is a natural race, so a clean run proves only itself.
r=$(new_repo)
_shims="$(mktemp -d "$ROOT/no-net-concurrent-install.XXXXXX")" || fixture_die "cannot create a scratch dir for the network shim"
printf '#!/bin/sh\necho "curl: network blocked in test" >&2\nexit 6\n' >"$_shims/curl"
chmod +x "$_shims/curl"
_reached="$r/git-reached"
_go="$r/git-go"
...
GST_V0_5_0_POST_COMMIT_RACE
    chmod +x "$r/.githooks/post-commit"
    (
        cd "$r" && GST_LABEL=1 PATH="$_shims:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh install >"$r/out1" 2>&1
        echo $? >"$r/rc1"
    ) &
    _p1=$!
    (
        cd "$r" && GST_LABEL=2 PATH="$_shims:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh install >"$r/out2" 2>&1
        echo $? >"$r/rc2"
    ) &
    _p2=$!
    ...
    wait "$_p1" 2>/dev/null
    wait "$_p2" 2>/dev/null
    _rc1="$(cat "$r/rc1" 2>/dev/null)"
    case "$_rc1" in '' | *[!0-9]*) _rc1=1 ;; esac
    _rc2="$(cat "$r/rc2" 2>/dev/null)"
    case "$_rc2" in '' | *[!0-9]*) _rc2=1 ;; esac
...
_out=$(cd "$r" && LC_ALL=fr_FR.UTF-8 PATH="$_shims:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh install 2>&1)
_rc=$?
wait "$_releaser" 2>/dev/null
_hp=$(cd "$r" && git config core.hooksPath 2>/dev/null)
if [ "$_rc" -ne 0 ]; then
...
# With GIT_DIR exported, git names the full config path, so a repository path containing "File
# exists" must not read as contention; GIT_DIR is exported into install's own environment.
...
_rc=$?
if [ "$_rc" -eq 0 ]; then
    fail "the commit went through: $_out"
...
_rc=$?
if [ "$_rc" -ne 0 ]; then
    fail "a clean lint was blocked (rc $_rc): $_out"
...
_rc=$?
if [ "$_rc" -ne 0 ]; then
    fail "the retry was still blocked (rc $_rc): $_out"
elif [ ! -f "$r/lint.argv.2" ]; then
    fail "the retry never re-ran the linter"
elif [ "$(commits "$r")" != 2 ]; then
    fail "exited 0 without committing"
else pass; fi
...
_rc=$?
if [ "$_rc" -ne 0 ]; then
    fail "a cold cache blocked a non-Swift commit (rc $_rc): $_out"
elif [ "$(commits "$r")" != 2 ]; then
    fail "exited 0 without committing"
else pass; fi
...
_rc=$?
if [ "$_rc" -eq 0 ]; then
    fail "an unrunnable gate was skipped: $_out"
elif [ "$(commits "$r")" != 1 ]; then
    fail "refused, and committed anyway"
else
    case "$_out" in
...
_rc=$?
if [ "$_rc" -ne 0 ]; then
    fail "blocked with no gates present (rc $_rc): $_out"
elif [ "$(commits "$r")" != 2 ]; then
    fail "exited 0 without committing"
else pass; fi
...
_rc=$?
if [ "$_rc" -ne 0 ]; then
    fail "an offline post-commit failed the commit (rc $_rc): $_out"
else
    case "$_out" in
...
_out=$(cd "$r" && PATH="$r/curl-shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh add swiftlint@0.63.2 2>&1)
_rc=$?
...
# Markers landing while add is fetching must be caught by the check add makes after taking its
# lock; the curl shim writes them on its first call and snapshots the file.
r=$(new_repo)
...
_tmpdir="$r/tmp'; touch \"$_sentinel\"; echo '"
mkdir -p "$_tmpdir"
_out=$(cd "$r" && PATH="$r/curl-shim:$PATH" TMPDIR="$_tmpdir" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh add swiftlint@0.63.2 2>&1)
_rc=$?
if [ -e "$_sentinel" ]; then
...
# A repo renamed away while add waits at its pins lock must fail fast naming the vanished directory;
# the pause is narrowed to grubstake.tools.lock because install_tool's cache lock comes first.
r=$(new_repo)
...
# An untraversable ancestor of the repo must read as a permission failure, not a removed repo; the
# mkdir shim flips the gate only at the pins lock, after add's download.
r=$(new_gated_repo)
_gate="$(dirname "$r")"
fake_release "$r" 1.0.0 >/dev/null
...
# `add` must never collapse an unreadable pins file to the one new pin; validate_pins' own read may
# refuse first, so the grep-shim test below covers add's rewrite without that dependency.
r=$(new_repo)
pins "$r" "periphery 3.7.4 $SHA_A $SHA_A
swiftformat 0.61.1 $SHA_B $SHA_B"
_before=$(cat "$r/grubstake.tools")
...
# The grep shim makes add's first rewrite grep select nothing, as a real empty match does, so only
# the pin-count check can refuse the dropped pins; the sentinel proves the shim fired.
r=$(new_repo)
pins "$r" "periphery 3.7.4 $SHA_A $SHA_A
swiftformat 0.61.1 $SHA_B $SHA_B"
_before=$(cat "$r/grubstake.tools")
...
fake_release "$r" 0.63.2 >/dev/null
_pins_path="$r/grubstake.tools"
_out=$(cd "$r" && PATH="$r/curl-shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh add swiftlint@0.63.2 notatool@1.0.0 2>&1)
_rc=$?
_after=$(cat "$_pins_path" 2>/dev/null || echo '')
if [ "$_rc" -eq 0 ]; then
    fail "add exited 0 despite a later spec (notatool) failing: $_out"
elif ! printf '%s\n' "$_after" | grep -qE '^swiftlint[[:space:]]+0\.63\.2[[:space:]]'; then
    fail "the earlier spec's pin (swiftlint 0.63.2) did not survive the later spec's failure. pins file now:
$_after"
...
# both tools without a second fixture or a real network fetch. Every pin check below matches the
# whole line -- name, version, and both 64-hex sha columns -- so a regression that records the
# right tool name with the wrong version, or a real version with an empty ("-") or malformed sha
# column, cannot read as "present" the way a bare name-anchored grep would.
r=$(new_repo)
_msrc="$r/multi-src"
...
_pins_path="$r/grubstake.tools"
_out=$(cd "$r" && PATH="$r/curl-shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh add swiftlint@0.63.2 swiftformat@0.61.1 2>&1)
_rc=$?
if [ "$_rc" -ne 0 ]; then
    fail "add exited non-zero over two valid specs: $_out"
elif ! grep -qE '^swiftlint[[:space:]]+0\.63\.2[[:space:]]+[0-9a-f]{64}[[:space:]]+[0-9a-f]{64}$' "$_pins_path" 2>/dev/null; then
    fail "swiftlint's pin is not 0.63.2 with a real sha256 in both platform columns: $(grep '^swiftlint' "$_pins_path" 2>/dev/null)"
elif ! grep -qE '^swiftformat[[:space:]]+0\.61\.1[[:space:]]+[0-9a-f]{64}[[:space:]]+[0-9a-f]{64}$' "$_pins_path" 2>/dev/null; then
    fail "swiftformat's pin is not 0.61.1 with a real sha256 in both platform columns: $(grep '^swiftformat' "$_pins_path" 2>/dev/null)"
...
# Simulates a second add's mkdir reclaiming $_lockdir between this run's own rmdir and disarm; guarded to fire once so a repeat rmdir on this same path does not re-signal the parent.
r=$(new_repo)
fake_release "$r" 1.0.0 >/dev/null
_lockdir="$r/grubstake.tools.lock"
...
_rc=$?
if [ "$_rc" -ne 0 ]; then
    fail "add exited $_rc on a cold cache: $_out"
...
it "add pins a tool whose verified entry is already cached"
# add's hash loop always fetches both platforms, so any third curl call is install downloading again.
r=$(new_repo)
fake_release "$r" 1.0.0 >/dev/null
_pins_path="$r/grubstake.tools"
_out1=$(cd "$r" && PATH="$r/curl-shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh add swiftlint@1.0.0 2>&1)
_rc1=$?
grep -v '^swiftlint[[:space:]]' "$_pins_path" >"$_pins_path.tmp" 2>/dev/null && mv "$_pins_path.tmp" "$_pins_path" || fixture_die "cannot drop the swiftlint pin"
mv "$r/curl-shim/curl" "$r/curl-shim/curl-real"
_curllog="$r/curl-calls"
: >"$_curllog"
cat >"$r/curl-shim/curl" <<SHIM
#!/bin/sh
echo x >> "$_curllog"
exec "$r/curl-shim/curl-real" "\$@"
SHIM
chmod +x "$r/curl-shim/curl" || fixture_die "cannot make the counting curl shim executable"
_out2=$(cd "$r" && PATH="$r/curl-shim:$PATH" GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh add swiftlint@1.0.0 2>&1)
_rc2=$?
_calls=$(wc -l <"$_curllog" | tr -d ' ')
if [ "$_rc1" -ne 0 ]; then
    fail "first add exited $_rc1, so this proves nothing: $_out1"
elif [ "$_rc2" -ne 0 ]; then
    fail "second add exited $_rc2 despite an already-verified cache entry: $_out2"
elif ! grep -qE '^swiftlint[[:space:]]+1\.0\.0[[:space:]]' "$_pins_path" 2>/dev/null; then
    fail "second add exited 0 but grubstake.tools has no swiftlint pin line: $(cat "$_pins_path" 2>/dev/null)"
elif [ "$_calls" -ne 2 ]; then
    fail "second add invoked curl $_calls time(s), expected exactly 2 (the hash loop only): $_out2"
else
    _bin=$(cd "$r" && GRUBSTAKE_CACHE="$r/.cache" ./grubstake.sh path swiftlint)
    _brc=$?
    if [ "$_brc" -ne 0 ] || [ ! -x "$_bin" ]; then
        fail "path did not resolve an executable after re-pinning an already-cached tool (rc $_brc): $_bin"
    else
        pass
    fi
fi
...
_out=$(cd "$_gs/repo" && PATH="$_gs/bin:$PATH" ./test/scan-for-leaks.sh 2>&1)
_rc=$?
...
_out=$(cd "$_lf/repo" && PATH="$_lf/bin:$PATH" ./test/scan-for-leaks.sh 2>&1)
_rc=$?
case "$_out" in
...
_out=$(cd "$_lg/repo" && PATH="$_lg/bin:$PATH" ./test/scan-for-leaks.sh --all 2>&1)
_rc=$?
case "$_out" in
...
# #137: asserting only that the stamp's second line came back empty proved nothing answered, which an offline machine also produces unfixed; the shim below proves no dial-out was even attempted.
r=$(adopted_repo)
...
for _f in commit-msg commit-msg.d/scan-for-leaks; do
    [ -f "$REPO/.githooks/$_f" ] || {
        fail "this repo has no .githooks/$_f, so its message tier lost three shapes"
        _f=""
        break
    }
...
# Order is the property, so the SHA capture ("git rev-parse HEAD") must precede the dispatch
# ("gh workflow run <name>"); the literal word "headSha" must appear in the dispatch's own step. A
# reworded enforcement sentence cannot be detected, so only the word's presence is pinned.
_wf="$REPO/.github/workflows/network.yml"
